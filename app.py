...
def predict():
    if request.method == "POST":
        # Extract and process the input features
        date_dep = request.form["Dep_Time"]
        Journey_day = int(pd.to_datetime(date_dep, format="%Y-%m-%dT%H:%M").day)
        Journey_month = int(pd.to_datetime(date_dep, format="%Y-%m-%dT%H:%M").month)
        ...
        airline_features = airline_mapping.get(airline, [0] * 11)

        Source = request.form["Source"]
        source_mapping = {'Delhi': [1, 0, 0, 0], 'Kolkata': [0, 1, 0, 0], 'Mumbai': [0, 0, 1, 0], 'Chennai': [0, 0, 0, 1]}
        source_features = source_mapping.get(Source, [0] * 4)

        Destination = request.form["Destination"]
        destination_mapping = {'Cochin': [1, 0, 0, 0, 0], 'Delhi': [0, 1, 0, 0, 0], 'New Delhi': [0, 0, 1, 0, 0], 'Hyderabad': [0, 0, 0, 1, 0], 'Kolkata': [0, 0, 0, 0, 1]}
        destination_features = destination_mapping.get(Destination, [0] * 5)
